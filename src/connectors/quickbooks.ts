...
export async function createOrUpdatePurchaseOrder(
  ...
  if (externalId) {
    ...
    const result = await qbRequest<Record<string, QBEntity>>("POST", "purchaseorder?operation=update&minorversion=70&include=enhancedAllCustomFields", payload);
  ...
  const result = await qbRequest<Record<string, QBEntity>>("POST", "purchaseorder?minorversion=70&include=enhancedAllCustomFields", safeData);
  const resultKey = Object.keys(result).find((k) => k !== "time");
